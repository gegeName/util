...
import com.simple.mylibrary.paging.PageStateHandler
...
class StateLayout @JvmOverloads constructor(
    ...
    defStyleAttr: Int = 0,
) : FrameLayout(context, attrs, defStyleAttr), PageStateHandler {
    ...
    init {
        if (attrs != null) {
            context.withStyledAttributes(attrs, R.styleable.StateLayout) {
                ...
                mEmptyLayoutId = getResourceId(R.styleable.StateLayout_stlEmptyLayout, mEmptyLayoutId)
                mErrorLayoutId = getResourceId(R.styleable.StateLayout_stlErrorLayout, mErrorLayoutId)
    ...
    override fun showLoading() = setState(State.LOADING)
    ...
    fun setStateBackgroundResource(@DrawableRes resId: Int) {
        ...
        mLoadingView?.let { if (resId != 0) it.setBackgroundResource(resId) else it.background = null }
        mEmptyView?.let { if (resId != 0) it.setBackgroundResource(resId) else it.background = null }
        mErrorView?.let { if (resId != 0) it.setBackgroundResource(resId) else it.background = null }
    }
    ...
    private fun ensureEmptyView() {
        ...
        addView(v, buildStateLp(v.layoutParams))
        v.isVisible = false
        v.findViewById<TextView>(R.id.tvPageStateEmpty)?.let { tv -> mEmptyText?.let { tv.text = it } }
        mEmptyView = v
    }
    ...
    private fun ensureErrorView() {
        ...
        addView(v, buildStateLp(v.layoutParams))
        v.isVisible = false
        v.findViewById<TextView>(R.id.tvPageStateError)?.let { tv -> mErrorText?.let { tv.text = it } }
