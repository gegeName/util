...
class ShapeDrawableBuilder(
    ...
    init {
        ...
        readAttr(ta, R.attr.shape_solidPressedColor_L) { solidPressedColor = ta.getColor(it, solidColor) }
        readAttr(ta, R.attr.shape_solidCheckedColor_L) { solidCheckedColor = ta.getColor(it, solidColor) }
        readAttr(ta, R.attr.shape_solidDisabledColor_L) { solidDisabledColor = ta.getColor(it, solidColor) }
        readAttr(ta, R.attr.shape_solidFocusedColor_L) { solidFocusedColor = ta.getColor(it, solidColor) }
        readAttr(ta, R.attr.shape_solidSelectedColor_L) { solidSelectedColor = ta.getColor(it, solidColor) }
        ...
        readAttr(ta, R.attr.shape_solidGradientStartColor_L) { solidGradientStartColor = ta.getColor(it, 0) }
        readAttr(ta, R.attr.shape_solidGradientCenterColor_L) { solidGradientCenterColor = ta.getColor(it, 0) }
        readAttr(ta, R.attr.shape_solidGradientEndColor_L) { solidGradientEndColor = ta.getColor(it, 0) }
        readAttr(ta, R.attr.shape_solidGradientOrientation_L) { solidGradientOrientation = ta.getInt(it, 0) }
        readAttr(ta, R.attr.shape_solidGradientType_L) { solidGradientType = ta.getInt(it, 0) }
        readAttr(ta, R.attr.shape_solidGradientRadius_L) { solidGradientRadius = ta.getDimension(it, 0f) }
        readAttr(ta, R.attr.shape_solidGradientCenterX_L) { solidGradientCenterX = ta.getFloat(it, 0.5f) }
        readAttr(ta, R.attr.shape_solidGradientCenterY_L) { solidGradientCenterY = ta.getFloat(it, 0.5f) }
        readAttr(ta, R.attr.shape_solidGradientUseLevel_L) { solidGradientUseLevel = ta.getBoolean(it, false) }
        ...
        readAttr(ta, R.attr.shape_bottomLeftRadius_L) { bottomLeftRadius = ta.getDimension(it, -1f) }
        readAttr(ta, R.attr.shape_bottomRightRadius_L) { bottomRightRadius = ta.getDimension(it, -1f) }

        readAttr(ta, R.attr.shape_strokeColor_L) { strokeColor = ta.getColor(it, 0) }
        readAttr(ta, R.attr.shape_strokePressedColor_L) { strokePressedColor = ta.getColor(it, strokeColor) }
        readAttr(ta, R.attr.shape_strokeCheckedColor_L) { strokeCheckedColor = ta.getColor(it, strokeColor) }
        ...
        readAttr(ta, R.attr.shape_strokeGradientStartColor_L) { strokeGradientStartColor = ta.getColor(it, 0) }
        readAttr(ta, R.attr.shape_strokeGradientCenterColor_L) { strokeGradientCenterColor = ta.getColor(it, 0) }
        readAttr(ta, R.attr.shape_strokeGradientEndColor_L) { strokeGradientEndColor = ta.getColor(it, 0) }
        readAttr(ta, R.attr.shape_strokeGradientOrientation_L) { strokeGradientOrientation = ta.getInt(it, 0) }
        ...
        return GradientDrawable().apply {
            ...
            if (solidGradientStartColor != 0 && solidGradientEndColor != 0) {
                ...
                colors = if (solidGradientCenterColor != 0)
                    intArrayOf(solidGradientStartColor, solidGradientCenterColor, solidGradientEndColor)
                else
            ...
            if (strokeSize > 0) {
                val finalStrokeColor = if (strokeGradientStartColor != 0 && strokeGradientEndColor != 0) {
    ...
    private fun applyRingAttrs(drawable: GradientDrawable) {
        runCatching {
            val stateField = drawable.javaClass.getDeclaredField("mGradientState").apply { isAccessible = true }
            val state = stateField.get(drawable)
            val cls = state.javaClass
            if (innerRadius >= 0) {
                cls.getDeclaredField("mInnerRadius").apply { isAccessible = true }.setInt(state, innerRadius)
            }
            cls.getDeclaredField("mInnerRadiusRatio").apply { isAccessible = true }
                .setFloat(state, innerRadiusRatio)
            if (thickness >= 0) {
                cls.getDeclaredField("mThickness").apply { isAccessible = true }.setInt(state, thickness)
            }
