...
class BorderedImageDrawable(
    ...
    gradientVertical: Boolean = false,
) : Drawable() {

    @Px val borderWidth: Float = borderWidth
    @ColorInt val borderColor: Int = borderColor
    @Px val cornerRadius: Float = cornerRadius
    val gradientColors: IntArray? = gradientColors
    val gradientVertical: Boolean = gradientVertical
        ...
        if (gradientColors != null) {
            ...
            if (cachedShader == null || cachedW != w || cachedH != h) {
                cachedShader = if (gradientVertical) {
                    LinearGradient(0f, 0f, 0f, h.toFloat(), gradientColors, null, Shader.TileMode.CLAMP)
                } else {
                    LinearGradient(0f, 0f, w.toFloat(), 0f, gradientColors, null, Shader.TileMode.CLAMP)
                }
                cachedW = w; cachedH = h
            }
